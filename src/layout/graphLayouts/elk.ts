...
import { Graph } from "@/dataManager/types";

// import { sizes } from "./nodeSizes";
import { Panel } from "@/dataManager/store";
...
const toElkFormat = (graph: Graph<any>): ElkNode => {
  ...
  const subtneworksAndMachinesELK = subtneworksAndMachines.map((node) => {
    ...
    if (node.type === "machine") {
      return {
        id: node.id,
        width: 200, //sizes.machine.width,
        height: 100, //sizes.machine.height,
        x: node.position.x,
        y: node.position.y,
        children: [],
      ...
      return {
        id: node.id,
        width: 100, // sizes.subnetwork.width,
        height: 100, //sizes.subnetwork.height,
        x: node.position.x,
        y: node.position.y,
        children: [],
        edges: edgesFromThisNode.length !== 0 ? edgesFromThisNode : [],
...
export const elkLayoutForce = async (
  ...
  const root = await elk(graph, {
    layoutOptions: {
      ...
      "spacing.nodeNode": "40", // Default 20
      "force.iterations": "200", // Default 300 && fait lagguer
