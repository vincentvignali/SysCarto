...
import { runEdgesPositionning } from "@/layout/nodeLayout/edges";
import { runNodesInnerPosition } from "@/layout/nodeLayout/nodes";
  ...
  reactFlowInstance.setNodes((nodes: Node<any>[]): Node<any>[] => {
    return nodes.map((node) => {
      if (node.type === "machine") {
        ...
        return node;
      } else if (node.type === "subnetwork" || node.type === "meta-service") {
        node.draggable = true;
        node = {
          ...node,
          style: {
